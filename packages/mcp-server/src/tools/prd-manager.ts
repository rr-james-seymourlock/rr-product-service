...
interface UserStory {
  ...
  status: 'not_started' | 'research' | 'in_progress' | 'review' | 'completed' | 'blocked';
...
export class PRDManager {
  ...
  static async generateImplementationPrompts(filename: string): Promise<string[]> {
    ...
    const readyStories = prd.userStories.filter((s) => s.status === 'not_started');
  ...
  static async getImprovementSuggestions(filename: string): Promise<string[]> {
    ...
    const vagueStories = prd.userStories.filter((s) => s.acceptanceCriteria.length < 2);
    if (vagueStories.length > 0) {
      suggestions.push(`${vagueStories.length} stories need more detailed acceptance criteria`);
    }

    const blockedStories = prd.userStories.filter((s) => s.status === 'blocked');
    if (blockedStories.length > 0) {
      suggestions.push(`${blockedStories.length} stories are blocked and need attention`);
    }
  ...
  static async getProjectStatus(filename: string): Promise<{
    ...
    const inProgress = prd.userStories.filter((s) => s.status === 'in_progress');
    const nextPending = prd.userStories.filter((s) => s.status === 'not_started').slice(0, 3);
  ...
  static async updateCustomPhase(
    ...
    if (updates.name && updates.name !== phase.name) {
      if (prd.customPhases.some((p) => p.name === updates.name && p.id !== phaseId)) {
  ...
  static async deleteCustomPhase(
    ...
    if (phase.userStoryIds.length > 0) {
      if (reassignToPhaseId) {
        ...
        if (!targetPhase) {
          throw new Error(`Target phase ${reassignToPhaseId} not found for reassignment`);
        }
  ...
  private static assessComplexity(criteria: string[]): UserStory['estimatedComplexity'] {
  ...
  private static updateProgress(prd: StructuredPRD): void {
    const completed = prd.userStories.filter((s) => s.status === 'completed').length;
    const blocked = prd.userStories.filter((s) => s.status === 'blocked').length;
    ...
    prd.progress.overall =
      prd.userStories.length > 0 ? Math.round((completed / prd.userStories.length) * 100) : 0;
  }
...
function createListPRDsTool(server: McpServer) {
  return server.tool('list_prds', 'List all Product Requirements Documents', async () => {
...
function createGetImplementationPromptsTool(server: McpServer) {
  return server.tool(
    ...
    async ({ state }) => {
      const prompts = await PRDManager.generateImplementationPrompts(state.filename);
...
function createGetImprovementSuggestionsTool(server: McpServer) {
  return server.tool(
    ...
    async ({ state }) => {
      const suggestions = await PRDManager.getImprovementSuggestions(state.filename);
